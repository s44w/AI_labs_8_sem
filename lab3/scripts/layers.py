...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(dx_hat * (x - sample_mean) * -0.5 * (inv_std**3), axis=0)
    dmean = np.sum(dx_hat * -inv_std, axis=0) + dvar * np.mean(
        -2.0 * (x - sample_mean), axis=0
    )

    dx = dx_hat * inv_std + dvar * 2.0 * (x - sample_mean) / N + dmean / N
        ...
        for f in range(F):
            for i in range(H_out):
                ...
                for j in range(W_out):
                    ...
                    window = x_padded[n, :, hs:he, ws:we]
                    dw[f] += window * grad
                    dx_padded[n, :, hs:he, ws:we] += w[f] * grad
        ...
        for c in range(C):
            for i in range(H_out):
                ...
                for j in range(W_out):
                    ws = j * stride
                    we = ws + pool_width
                    out[n, c, i, j] = np.max(x[n, c, hs:he, ws:we])
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dsum = np.sum(dx_group, axis=(2, 3, 4), keepdims=True)
    dvar = np.sum(dx_group * (x_group - mean), axis=(2, 3, 4), keepdims=True)
    dx = (
        (group_size * dx_group - dsum - (x_group - mean) * (inv_std**2) * dvar)
        * inv_std
        / group_size
    )
